...
IExpression* NodeIterator::DropLeft(IExpression* exp)
{
	...
	for (;;)
	{
		...
		if (const auto uExp = dynamic_cast<const UnaryExpressionBase*>(node))
			node = uExp->GetArg().get();

		if (const auto uExp = dynamic_cast<const BinaryExpressionBase*>(node))
			node = uExp->GetLeftArg().get();
	}
}
//------------------------------------------------------------------------------
