...
symb::Expression AstWalker::operator()(ast::Operation const& x, symb::Expression lhs) const
{
    ...
    switch (x.operator_)
    {
        case '+': return symb::MakeExpression<symb::Sum>(std::move(lhs), std::move(rhs));
        case '-': return symb::MakeExpression<symb::Diff>(std::move(lhs), std::move(rhs));
        case '*': return symb::MakeExpression<symb::Prod>(std::move(lhs), std::move(rhs));
        case '/': return symb::MakeExpression<symb::Div>(std::move(lhs), std::move(rhs));
        case '^': std::cerr<<"pow operation is undefined. Terminated"<<std::endl;
    }
    BOOST_ASSERT(0);
}
//------------------------------------------------------------------------------
